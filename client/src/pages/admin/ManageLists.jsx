...
import { Trash2, Plus, Globe, Package, Tag, Image as ImageIcon, RotateCcw, Upload, Check } from 'lucide-react';
...
const ManageLists = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [countryRes, productRes, statusRes] = await Promise.all([
    ...
    const handleAddCountry = async (e) => {
        ...
        try {
            await settingsAPI.addCountry({ name: newCountry });
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center border-b pb-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Manage Lists & Application Branding</h1>
                    <p className="text-gray-600 text-sm">Configure system settings, master data lists, and global application branding</p>
                </div>
            </div>

            {/* Application Branding Management Card */}
            <div className="card bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex items-center justify-between border-b pb-4 mb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-xl font-bold text-gray-900">Application Branding</h2>
                    </div>
                    <button
                ...
                <div className="card">
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                        {countries.map((country) => (
                            <div key={country.id} className="flex items-center justify-between p-2 bg-gray-50 rounded border border-gray-100 group hover:bg-white hover:border-gray-200 transition-colors">
                        ...
                        {countries.length === 0 && (
                            <p className="text-gray-500 text-sm text-center italic py-2">No countries added in master list</p>
                        )}
                    </div>
                </div>
