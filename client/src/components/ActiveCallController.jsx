...
import { PhoneCall, Mic, MicOff, Volume2, VolumeX, PhoneOff, CheckCircle2, ShieldAlert } from 'lucide-react';
...
const ActiveCallController = () => {
  ...
  // Subscribe to Native Android Telecom Bridge Events
  useEffect(() => {
    const handleNativeCallEvent = (e) => {
      const detail = e.detail || {};
      console.log('[ActiveCallController] Native event received:', detail);
  ...
  const handleEndCall = () => {
    if (window.AndroidCRM?.endCall) {
      window.AndroidCRM.endCall(callSession?.callId || '');
    } else {
      setCallSession(null);
    }
  };
  ...
  if (callSession) {
    ...
    return (
      <div className="fixed bottom-3 left-3 right-3 md:left-auto md:right-6 md:max-w-md z-50 animate-in slide-in-from-bottom-5 duration-300">
        <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700/80 rounded-2xl p-4 text-white shadow-2xl">
